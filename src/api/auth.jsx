...
async function login(credentials, register=false) {
    return await axios.post(register? "/register" : "/login", credentials).then((res) => {
        if(res.status === 200) {
...
function vefyu() {
    return axios.get("/auth/verify").then((res) => {
        if(res.status === 200) {
            localStorage.setItem("user", JSON.stringify(res.data));
            if(!inProgress) {
                inProgress = true;
...
async function getNotes(noteId=null) {
    return await axios.get(noteId ? `/api/notes/${noteId}` : "/api/notes").then((res) => {
        if(res.status === 200) {
            return res.data;
        }
    }).catch((e) => {
...
async function deleteNote(noteId) {
    return await axios.delete(`/api/notes/${noteId}`).then((res) => {
...
async function newNote(note) {
    return await axios.post("/api/notes", {...note}).then((res) => {
        return res.status === 200;
    }).catch((e) => {
        console.log(e);
        notify(e.toString(), "failed");
...
async function logoutUser() {
    return await axios.get("/auth/logout").then((res) => {
        return res.status === 200;
    }).catch((e) => {
        console.log(e);
        notify(e.toString(), "failed");
